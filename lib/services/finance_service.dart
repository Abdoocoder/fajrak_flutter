...
class FinanceService {
  ...
  static Future<Map<String, dynamic>> fetchMonthlyFinancialSummary({
    ...
    final response = await _db.rpc(
      ...
      params: {'p_user_id': _uid, 'p_year': year, 'p_month': month},
    );
    return Map<String, dynamic>.from(response as Map);
  }

  /// جلب ملخص لوحة المعلومات المالية باستخدام RPC المركزية
  static Future<Map<String, dynamic>> fetchFinancialDashboard(
    double usdToLocalRate,
  ) async {
    final response = await _db.rpc(
      'get_financial_dashboard',
      params: {'p_user_id': _uid, 'p_usd_to_local_rate': usdToLocalRate},
    );
    return Map<String, dynamic>.from(response as Map);
  }
