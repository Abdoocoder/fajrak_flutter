...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _totalAccountsBalance =
          _accounts.fold(0.0, (sum, acc) => sum + (acc['balance'] as double? ?? 0.0));
    ...
    try {
      ...
      final results = await Future.wait<dynamic>([
        ...
        Supabase.instance.client
            .from('transactions')
            .select(
                'id, type, amount, category, description, transaction_date, original_amount, original_currency')
            .eq('user_id', user.id)
        ...
        FinanceService.fetchMonthlyFinancialSummary(year: now.year, month: now.month)
      ...
      final txExpenses =
          (monthly['expenses'] as num?)?.toDouble() ?? localExpenses;
      ...
      final monthlyDebtCommitments = debts
          .where((d) =>
              d['auto_deduct'] == true && (d['debt_type'] ?? 'owed') == 'owed')
          .fold(0.0,
      ...
      if (mounted) {
        setState(() {
          ...
          _goalsTarget = goals.fold(
              0.0, (a, g) => a + (g['target_amount'] as num).toDouble());
                    ...
                    : Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Expanded(
                                  child: DashboardHeader(name: _name)),
                              const SizedBox(width: 8),
                              GestureDetector(
                                onTap: () =>
                                    showDashboardCustomizer(context),
                                child: Container(
...
class _AccountsBalanceCard extends StatelessWidget {
  ...
  String _fmt(double n) =>
      n.abs() % 1 == 0 ? n.abs().toStringAsFixed(0) : n.abs().toStringAsFixed(2);
...
class CardSkeleton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SkeletonLoader(
        width: double.infinity, height: height, borderRadius: 18);
  }
}
